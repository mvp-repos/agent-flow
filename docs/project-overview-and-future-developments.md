...
**Workflow diagram:** The full workflow (intake, clarity gate, repo selection, branch, PRD/draft, checks, human review, approval, commit, push, PR) is documented in the flowchart image in the repository root (`base-agentflow-diagram.webp`). It aligns with the state machine (Section 4) and the implementation in `WorkflowEngine.cs`. The "next steps" in Section 8 follow the same order (draft → checks → approval → PR).
...
- **Workflow diagram:** Flowchart image in the repository root (`base-agentflow-diagram.webp`) shows intake → approval → PR → done. Keep it aligned with `RunState.cs` and `WorkflowEngine.cs`.
...
| Workflow diagram                 | Flowchart image in repo root (`base-agentflow-diagram.webp`)                                                                                               |
